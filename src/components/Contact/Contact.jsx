import { Link } from "react-router-dom";
import "./contact.css";

export const Contact = () => {
  return (
    <>
    <div className="contact-container">
     <h2 className="d-flex justify-content-center contact-title">Reservas y pedidos</h2> 
     <div className="d-flex justify-content-evenly main-container">
        <div className="d-flex flex-column justify-content-center">
          <Link to='https://api.whatsapp.com/send?phone=34930381911&text=Hola%20,quiero%20hacer%20una
%20reserva:%20DIA:%20HORARIO:%20PERSONAS:%20' target='_blank' className="text-decoration-none text-center">
          <i className="bi bi-whatsapp whatsapp-icon"></i>
          <h3>Contáctanos por whatsapp</h3>
          </Link>
          <p>Horario de atención: </p>
        </div>
        <div className="d-flex flex-column justify-content-center">
          <Link to='tel:930381911' target='_blank' className="text-decoration-none text-center">
          <i class="bi bi-telephone-outbound phone-icon"></i>
          <h3>Contáctanos por teléfono</h3>
          </Link>
          <p>Horario de atención: </p>
        </div>
        <div className="d-flex flex-column justify-content-center">
          <Link to='tel:930381911' className="text-decoration-none text-center">
          <svg className='delivery-icon' fill="#b53a20" height="120px" width="120px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 453.68 450" xml:space="preserve" stroke="#b53a20" ><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC" stroke-width="2.722062"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M390.859,276.402c-34.639,0-62.819,28.18-62.819,62.818c0,34.637,28.181,62.816,62.819,62.816 c34.638,0,62.817-28.179,62.817-62.816C453.677,304.582,425.497,276.402,390.859,276.402z M390.859,388.037 c-26.919,0-48.819-21.899-48.819-48.816c0-26.918,21.9-48.818,48.819-48.818c26.918,0,48.817,21.9,48.817,48.818 C439.677,366.138,417.777,388.037,390.859,388.037z"></path> <path d="M390.859,267.704c14.744,0,28.898,4.449,40.934,12.865c3.171,2.216,7.534,1.443,9.748-1.725 c2.216-3.168,1.443-7.533-1.725-9.748c-14.4-10.07-31.329-15.393-48.957-15.393c-1.188,0-2.371,0.026-3.549,0.075 c-0.052-0.167-0.103-0.335-0.168-0.5c-16.414-41.715-36.896-81.137-61.234-117.937h20.352c3.866,0,7-3.134,7-7V96.009 c0-3.866-3.134-7-7-7h-25.058c-8.549,0-16.026,4.658-20.04,11.565c-12.347-16.156-25.492-31.739-39.429-46.706 c-2.635-2.829-7.064-2.986-9.894-0.353c-2.829,2.635-2.987,7.064-0.353,9.894c13.275,14.256,25.821,29.082,37.623,44.444 l-21.875,21.875c-1.313,1.313-2.05,3.093-2.05,4.95c0,1.857,0.737,3.637,2.05,4.95c21.35,21.349,33.107,49.739,33.107,79.939 c0,28.841-10.73,56.025-30.29,77.008l-49.898-14.477c-15.018-4.287-24.909-18.192-24.054-33.816 c0.485-8.874,4.398-17.029,11.017-22.961c0.534-0.479,1.085-0.932,1.643-1.372c0.596-0.317,1.148-0.723,1.639-1.211 c2.636-1.831,5.494-3.256,8.501-4.248h7.032c12.941,0,23.47-10.528,23.47-23.47c0-12.941-10.528-23.47-23.47-23.47h-6.643h-23.071 H53.941c-9.558,0-17.334,7.776-17.334,17.334s7.776,17.334,17.334,17.334h17.59c-13.936,6.695-26.751,15.552-37.943,26.262H22.051 c-2.127,0-4.139,0.967-5.467,2.629l-9.83,12.296c-1.344,1.68-1.84,3.885-1.346,5.979l2.709,11.482 c-2.766,4.772-5.286,9.728-7.53,14.862c-1.548,3.543,0.069,7.669,3.612,9.217c0.913,0.398,1.863,0.587,2.799,0.587 c2.698,0,5.268-1.569,6.418-4.199c4.436-10.152,10.059-19.539,16.642-28.052l11.909-13.091c0.314-0.345,0.574-0.723,0.808-1.113 c24.296-23.493,57.159-37.333,92.253-37.333c20.334,0,40.348,4.668,58.443,13.55c-6.785,7.864-10.776,17.682-11.353,28.223 c-1.215,22.194,12.845,41.95,34.161,48.034l53.863,15.597c0.132,0.038-0.132-0.03,0,0c0.664,0.202,1.352,0.305,2.04,0.305 c1.791,0,3.583-0.684,4.95-2.05c23.994-23.992,37.208-55.897,37.208-89.837c0-31.595-11.453-61.428-32.391-84.755l15.645-15.645 c30.637,41.849,55.88,87.515,75.396,136.431c-30.864,6.61-56.251,30.01-64.679,61.391l-2.823,9.188v0 c-3.218,10.345-7.844,17.121-13.748,20.141c-5.222,2.671-12.034,2.63-20.245-0.123c-0.055-0.019-0.11-0.029-0.165-0.046 l-73.346-25.298c-16.41-6.37-24.32-12.524-30.2-23.49c-0.154-0.287-0.331-0.553-0.517-0.81 c-14.789-25.535-42.395-42.762-73.965-42.762c-47.109,0-85.435,38.325-85.435,85.433c0,3.866,3.134,7,7,7s7-3.134,7-7 c0-39.388,32.045-71.433,71.435-71.433c39.388,0,71.433,32.045,71.433,71.433c0,3.866,3.134,7,7,7c3.866,0,7-3.134,7-7 c0-4.18-0.316-8.286-0.898-12.306c4.583,2.623,9.623,4.872,15.182,7.026c0.042,0.017,0.085,0.027,0.128,0.043 c0.04,0.015,0.078,0.034,0.119,0.048l73.722,25.428c0.054,0.018,0.108,0.024,0.162,0.041c5.596,1.866,10.868,2.801,15.788,2.801 c5.518,0,10.592-1.174,15.179-3.52c9.383-4.8,16.362-14.371,20.741-28.446c0.001-0.005,0.002-0.011,0.004-0.016 c0.001-0.003,0.002-0.005,0.003-0.008l2.868-9.335c0.025-0.083,0.049-0.167,0.072-0.25 C330.136,289.508,358.533,267.704,390.859,267.704z"></path> <path d="M390.858,324.005c-8.39,0-15.216,6.826-15.216,15.216c0,8.389,6.826,15.214,15.216,15.214 c8.39,0,15.215-6.825,15.215-15.214C406.073,330.83,399.248,324.005,390.858,324.005z"></path> <path d="M93.302,276.402c-34.638,0-62.818,28.18-62.818,62.818c0,34.637,28.18,62.816,62.818,62.816h0.001 c34.638,0,62.817-28.179,62.817-62.816C156.12,304.582,127.94,276.402,93.302,276.402z M93.303,388.036L93.303,388.036 c-26.92,0-48.819-21.899-48.819-48.816c0-26.918,21.9-48.818,48.818-48.818c26.918,0,48.818,21.9,48.818,48.818 C142.12,366.137,120.221,388.036,93.303,388.036z"></path> <path d="M93.301,324.005c-8.39,0-15.216,6.826-15.216,15.216c0,8.389,6.826,15.214,15.216,15.214s15.216-6.825,15.216-15.214 C108.516,330.83,101.691,324.005,93.301,324.005z"></path> </g> </g></svg>          
          <h3 className="" >Recoge en nuestro local</h3>
          </Link>
          <p className="delivery-text" >Horario de atención: </p>
        </div>
     </div>
     <p className="contact-confirmation">Confirmaremos su reserva o pedido tan pronto comprobemos la disponibilidad para la fecha y hora solicitadas.</p> 
     <p className="contact-confirmation mt-0 mb-5">Si la reserva o pedido no son confirmados, no tendrá validez alguna.</p>
    </div>
    </>
  )
}
